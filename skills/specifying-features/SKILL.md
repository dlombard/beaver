...
6. **Approval is explicit.** The spec header carries an approval mode: `self-approved` (it passed the review workflow below — the default when working autonomously) or `awaiting human sign-off` (when the user wants a checkpoint, or the feature is high-risk / hard to reverse). A spec is never silently "approved".
...
6. Write one file per feature into the project's spec location; absent a layout, default to `docs/specs/<id>-<slug>.md`.
